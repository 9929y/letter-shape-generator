...
- **Adjust** — 形状(Circle / Square / ASCII)加五根滑杆:**Density**(数量)· **Grain**(颗粒大小)· **Liveliness**(颤动与呼吸)· **Tempo**(循环节奏,仅 Loop)· **Spread**(散开范围),外加 **Shuffle**(换一版构图:同样的字、同样的颜色,粒子换一种排布)。一根滑杆背后写的是原来的两三个参数,而且是**按当前比例缩放**而不是写死一对值 —— 所以 Brickwork 那种粗颗粒被你推一下之后还是粗的
