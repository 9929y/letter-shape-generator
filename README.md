...
在 Hand 分组里点 **Turn on camera** 授权摄像头。画面只在这个标签页里跑:每帧被画进一张 160 × 120 的临时 canvas 做识别,用完即弃,不上传、不留存。预览框里蓝色的就是当前被认定为手的区域,底部标签显示 `Open palm` / `Fist` / 没找到手。
...
**Ring** 控制画布上要不要画那个很淡的手部指示圈(PNG 导出会带上它)。
